...
class PostController{
    ...
    static public function postData($schema, $table, $data){
        ...
        $response = PostModel::postData($schema, $table, $data);
	...
	/*=============================================
	Peticion POST para registrar usuarioooo
	=============================================*/

	static public function postRegister($schema, $table, $data, $suffix){
	...
	static public function postLogin($schema, $table, $data, $suffix){
		...
		$response = GetModel::getDataFilter($schema, $table, "*", $suffix."Email", $data[$suffix."Email"], null,null,null,null);
		...
		if(!empty($response)){
				...
				if($response[0]->{$suffix."Password"} == $crypt){
					...
					//$jwt = Connection::jwt($response[0]->{"docNumId"}, $response[0]->{$suffix."Email"});
